...
void sigmoid_wrapper(float *h_input, float *h_output, int N)
{
    ...
    sigmoid<<<grid, block>>>(d_input, d_output, N);
...
int main(int argc, char **argv) 
{
    ...
    // kernel function 
    sigmoid_wrapper(h_input, h_output, N);

    for (size_t i = 0; i < N; i++)
    {
        printf("src val: %f, corresponding sigmoid result: %f\n", h_input[i], h_output[i]);
    }
